...
export class MemoryManager {
  ...
  _compactForStorage(data) {
    return {
      ...
      // Persist grounding fields (P2) alongside the legacy projection so the
      // outcome loop can bucket archived ideas by grounding tier, and so the
      // LLM's self-report (llm_confidence) can be diffed against the grounded
      // confidence for calibration. Fields are undefined for pre-P2 runs.
      ideas: (data.ideas || []).map(i => ({
        title: i.title,
        type: i.type,
        confidence: i.confidence,
        llm_confidence: i.llm_confidence ?? null,
        evidence_score: typeof i.evidence_score === 'number' ? i.evidence_score : null,
        evidence_tier: i.evidence_tier ?? null,
      })),
    };
  }
