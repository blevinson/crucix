...
EARNINGS GATE — the EARNINGS_NEXT_7D block (when present) lists candidate tickers reporting within 7 days, with date + session (bmo/amc). For EVERY LONG/SHORT you draft:
  ...
  try {
    ...
    // was confirming-only and is dropped. So this is a single Sonnet completion
    // of a ~10-12KB prompt — seconds, not minutes. Timeout 240s gives generous
    // headroom for one large completion (the 15min was purely for the agentic
    // loop, which no longer exists).
    const completeOpts = { maxTokens: 4096, timeout: 240000 };
