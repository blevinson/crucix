...
function compactSweepForLLM(data, delta, previousIdeas) {
  ...
  // Retail trader sentiment (Stocktwits, ApeWisdom, Reddit WSB)
  if (data.stocktwits?.topByVolume?.length || data.apewisdom?.topMovers?.length || data.redditEnriched?.topTickers?.length) {
    const parts = [];

    // Stocktwits: top tickers with bullish% from user-tagged sentiment
    const stTop = (data.stocktwits?.topByVolume || []).slice(0, 8)
      .filter(t => t.total >= 5)
      .map(t => `${t.ticker} ${t.total}msg ${t.bullishPct != null ? t.bullishPct + '%bull' : 'n/a'}`);
    if (stTop.length) parts.push(`ST:${stTop.join(', ')}`);

    // ApeWisdom: top movers (biggest delta vs prior 24h)
    const awMovers = (data.apewisdom?.topMovers || []).slice(0, 6)
      .map(t => `${t.ticker} ${t.mentions}mentions ${t.deltaPct != null ? (t.deltaPct > 0 ? '+' : '') + t.deltaPct + '%' : ''}`);
    if (awMovers.length) parts.push(`AW_MOVERS:${awMovers.join(', ')}`);

    // Reddit enriched: top tickers by mention count across WSB/stocks/investing
    const rdTop = (data.redditEnriched?.topTickers || []).slice(0, 8)
      .filter(t => t.mentions >= 3)
      .map(t => {
        const subs = (t.subreddits || []).join('/');
        const bull = t.bullishPct != null ? ` ${t.bullishPct}%bull` : '';
        return `${t.ticker} ${t.mentions}x(${subs})${bull}`;
      });
    if (rdTop.length) parts.push(`REDDIT:${rdTop.join(', ')}`);

    if (parts.length) {
      sections.push(`\nRETAIL_SENTIMENT (cite specific tickers when issuing ideas):\n${parts.join('\n')}`);
    }
  }
