...
export class ClaudeCodeProvider extends LLMProvider {
  ...
  async complete(systemPrompt, userMessage, opts = {}) {
    ...
    try {
      ...
      // Opt-in tool use: caller passes mcpConfig (inline JSON) + allowedTools
      // (glob list). Without these we lock everything down with --disallowedTools '*'.
      if (opts.mcpConfig && opts.allowedTools && opts.allowedTools.length > 0) {
        args.push('--mcp-config', typeof opts.mcpConfig === 'string' ? opts.mcpConfig : JSON.stringify(opts.mcpConfig));
